'use strict';
console.log('%cWindows 12 网页版(GitHub: github.com/clay-psy)', 'background-image: linear-gradient(to right, #ad6eca, #3b91d8); border-radius: 8px; font-size: 1.3em; padding: 10px 15px; color: #fff; ');
const server = 'http://github.com/clay-psy';
const pages = {
    'get-title': '', // 获取标题
};
const page = $('html')[0];
...
let cms = {
    ...
    'desktop': [
        ...
        '<a onmousedown="window.open(\'https://github.com/clay-psy\');" win12_title="https://github.com/clay-psy" onmouseenter="showdescp(event)" onmouseleave="hidedescp(event)"><i class="bi bi-github"></i> 在 Github 中查看此项目</a>',
        arg => {
            if (edit_mode) {
...
let nts = {
    'feedback': {
        cnt: `<p class="tit">反馈</p>
            <p>我们非常注重用户的体验与反馈</p>
            <list class="new">
                <a class="a" onclick="window.open('https://github.com/clay-psy/');" win12_title="在浏览器新窗口打开链接" onmouseenter="showdescp(event)" onmouseleave="hidedescp(event)">在github上提交(需要github账户，会得到更高重视)</a>
            </list>`,
        btn: [
...
let apps = {
    setting: {
        ...
        theme_get: () => {
            ...
            // 实时获取主题
            $.get('http://github.com/clay-psy').then(cs => {
                cs.forEach(c => {
                    if (c.type == 'dir') {
                        $.get(c.url).then(cnt => {
                            $('#set-theme').html('');
                            cnt.forEach(cn => {
                                if (cn.name == 'theme.json') {
                                    $.getJSON('http://github.com/clay-psy' + cn.path).then(inf => {
                                        infjs = inf;
                                        if ($('#set-theme>loading').length)
                                            $('#set-theme').html('');
                                        $('#set-theme').append(`<a class="a act" onclick="apps.setting.theme_set('${c.name}')" style="background-image:url('http://github.com/clay-psy')">${c.name}</a>`);
                                    });
                                }
                            });
                        });
        ...
        theme_set: (infp) => {
            $.get('http://github.com/clay-psy' + infp).then(cnt => {
                console.log('http://github.com/clay-psy' + infp);
                cnt.forEach(cn => {
                    if (cn.name == 'theme.json') {
                        $.getJSON('http://github.com/clay-psy' + cn.path).then(inf => {
                            infjs = inf;
                            cnt.forEach(fbg => {
                                ...
                                if (fbg.name == infjs.bg) {
                                    $(':root').css('--bgul', `url('http://github.com/clay-psy')`);
    ...
    about: {
        ...
        get: () => {
            ...
            // 实时获取项目贡献者
            $.get('https://api.github.com/github.com/clay-psy').then(cs => {
            });
        },
        get_star: () => {
            apps.about.run_loading('#StarShow');
            const repoFullName = 'github.com/clay-psy';
        }
    },
    notepad: {
        init: () => {
...
let copilot = {
    ...
    init: () => {
        ...
        仅有以下信息供你使用来回答用户的问题。
        1.Windows 12 网页版是一个开源项目，由潘少岩原创, 使用 Html,css,js，在网络上模拟、创新操作系统
        2.项目的地址是https://github.com/clay-psy
        3.此项目使用EPL v2.0开源许可
...
document.getElementsByTagName('body')[0].onload = () => {
    ...
    $.getJSON('http://github.com/clay-psy').then(j=>{
        if(j.sp){
            $(':root').css('--bgul',j.bg);
            if(j.spth){
